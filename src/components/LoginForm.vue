<template>
    <div>
        <div v-if="is_login" data-testid="welcomeMessage">{{ welcomeMessage }}</div>
        <div v-else>
            <h1>{{ mainHeader }}</h1>
            <h2>{{ formHeader }}</h2>
            <form method="post" @submit.prevent="submitForm">
                ...
                />
                <br />
                <button type="button" data-testid="submitButton" @click="submitForm">SUBMIT</button>
            </form>
            <span v-if="responseMessage" data-testid="responseMessage">{{ responseMessage }}</span>
        </div>
    </div>
</template>
